...
if ( ! function_exists( 'kappscores_entry_footer' ) ) :
     ...
     */
    function kappscores_entry_footer() {
 ...
 * @return bool
 */
function kappscores_categorized_blog() {
    if ( false === ( $all_the_cool_cats = get_transient( 'kappscores_categories' ) ) ) {
        ...
        set_transient( 'kappscores_categories', $all_the_cool_cats );
    }

    if ( $all_the_cool_cats > 1 ) {
        // This blog has more than 1 category so kappscores_categorized_blog should return true.
        return true;
    } else {
        // This blog has only 1 category so kappscores_categorized_blog should return false.
        return false;
    }
}

/**
 * Flush out the transients used in kappscores_categorized_blog.
 */
function kappscores_category_transient_flusher() {
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
        return;
    }
    // Like, beat it. Dig?
    delete_transient( 'kappscores_categories' );
}
add_action( 'edit_category', 'kappscores_category_transient_flusher' );
add_action( 'save_post',     'kappscores_category_transient_flusher' );
