<?php
/**
 * kappscores functions and definitions.
 *
...
if ( ! function_exists( 'kappscores_setup' ) ) :
    /**
     ...
     */
    function kappscores_setup() {
        /*
         * Make theme available for translation.
         * Translations can be filed in the /languages/ directory.
         * If you're building a theme based on kappscores, use a find and replace
         * to change 'kappscores' to the name of your theme in all the template files.
         */
        ...
        add_image_size( 'kappscores-index-img', 1000, 550, true );
        ...
        add_theme_support( 'custom-background', apply_filters( 'kappscores_custom_background_args', array(
 ...
 */
function kappscores_resource_hints( $urls, $relation_type ) {
    if ( wp_style_is( 'kappscores-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
        $urls[] = array(
...
add_filter( 'wp_resource_hints', 'kappscores_resource_hints', 10, 2 );
 ...
 */
function kappscores_content_width() {
    $GLOBALS['content_width'] = apply_filters( 'kappscores_content_width', 640 );
}
 ...
 */
function kappscores_content_image_sizes_attr( $sizes, $size ) {
    $width = $size[0];
...
add_filter( 'wp_calculate_image_sizes', 'kappscores_content_image_sizes_attr', 10, 2 );
 ...
 */
function kappscores_header_image_tag( $html, $header, $attr ) {
    if ( isset( $attr['sizes'] ) ) {
        $html = str_replace( $attr['sizes'], '100vw', $html );
    }
    return $html;
}
add_filter( 'get_header_image_tag', 'kappscores_header_image_tag', 10, 3 );
 ...
 */
function kappscores_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'kappscores_post_thumbnail_sizes_attr', 10, 3 );
...
function kappscores_scripts() {
    ...
    wp_enqueue_style( 'kappscores-fonts', kappscores_fonts_url() );
...
add_action( 'wp_enqueue_scripts', 'kappscores_scripts' );
